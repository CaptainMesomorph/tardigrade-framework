...
{
    private readonly Rest.IRestClient _client;
    private readonly ITestOutputHelper _output;

    public GitHubClientTest(ITestOutputHelper output)
    {
        _output = output;
        ...
        client.AddDefaultHeader("User-Agent", "HttpRequestsSample");
        _client = new RestSharpClient(client);
    }

    [Fact]
    public void Retrieve_Exists_Success()
    {
        Response<object> response = _client.Get<object>("repos/dotnet/AspNetCore.Docs/branches");
        var content = response.Data?.ToString();
        _output.WriteLine($"{response.StatusCode}\n{content}");
    }
